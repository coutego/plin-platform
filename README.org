...
node packages/create-plin-app/bin/create-plin-app.js my-app
#+END_SRC
...
This project is published as two npm packages:

- *@coutego/plin-platform*: The core platform library
- *@coutego/create-plin-app*: CLI tool for scaffolding new projects
...
*** Publishing create-plin-app

#+BEGIN_SRC bash
cd packages/create-plin-app
npm publish --access public
#+END_SRC

*** Version Synchronization

When releasing a new version, update the version in both:
- ~package.json~ (root)
- ~packages/create-plin-app/package.json~

Also update the dependency version in ~packages/create-plin-app/bin/create-plin-app.js~.
