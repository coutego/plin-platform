...
These are also available via the nav-bar interface (~::inav/user-data~, ~::inav/user-actions~, ~::inav/user-widget~).
...
- *~manifest.edn~*: Plugin manifest that defines which plugins to load.

*** How It Works

When you use PLIN as an npm dependency, the server (~plin-platform.server~) acts as a virtual file server:
1. It serves framework files (~libs/...~) from the installed ~node_modules/plin-platform~ location.
...
*** Example 1: Replacing the App Shell (Custom Skin)
...
The key feature here is using ~::isession/user-data~ and ~::isession/user-actions~ to build a custom user widget that matches the skin's theme, rather than using the default ~::inav/user-widget~.
...
(def plugin
  (plin/plugin
   {:doc "A fancy, dark-themed replacement for the App Shell using user-data/user-actions pattern."
    :deps [iapp/plugin iauth/plugin ibread/plugin inav/plugin isession/plugin]
...
This works because both the default shell and this fancy shell implement the same interface (~i-app-shell~) and consume the same beans (~::inav/items~, ~::iauth/user~). The fancy shell uses ~::isession/user-data~ and ~::isession/user-actions~ to build a custom user widget that matches its dark theme.
