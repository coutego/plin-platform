...
(defonce routing-setup? (atom false))
...
;; --- Header Component ---
(defn header [app-name app-subtitle app-logo user-data user-actions sidebar-toggle!]
...
;; --- Main Content Area ---
(defn main-content [routes can? user-atom trail-data trail-actions sidebar-collapsed?]
    ...
    [:main {:class (str "flex-1 p-6 md:p-8 overflow-x-hidden transition-all duration-300 "
     ...
     (cond
       ...
       (try
         [Page]
         (catch :default e
           (js/console.error "AppShell: Error rendering Page:" e)
           [:div {:class "p-8 text-red-500"} 
            [:h2 {:class "text-xl font-bold"} "Render Error"]
            [:pre (str e)]])))]))

;; --- Overlay Wrapper Component ---
(defn overlay-wrapper [comp]
  [comp])

;; --- Main Layout Component ---
(defn app-layout [name subtitle logo routes overlays login-modal can? user-atom trail-data trail-actions sidebar-ui sidebar-toggle! sidebar-collapsed? user-data user-actions head-inject!]
  ...
  [:div {:class "min-h-screen text-slate-600"}
   ...
   [header name subtitle logo user-data user-actions sidebar-toggle!]
   ...
   [:div {:class "flex pt-16 min-h-screen"}
    ;; Sidebar (Injected)
    (when sidebar-ui [sidebar-ui])
    
    [main-content routes can? user-atom trail-data trail-actions sidebar-collapsed?]]
  ...
  (fn []
    (if root-component-fn
      (try
        ...
        (catch :default e
          (js/console.error "Error mounting app:" e)))
      (js/console.error "Error: root-component-fn is nil!"))))
