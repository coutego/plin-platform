  ...
  (r/atom
   {:config {:endpoint "https://openrouter.ai/api/v1/chat/completions"
    ...
    :chat {:messages []
           ...
           :current-stream ""
           :cost 0
           :last-plugin-error nil}  ;; {:error "message" :code "plugin code"}
...
(defn get-last-plugin-error []
  (get-in @state [:chat :last-plugin-error]))

(defn set-last-plugin-error!
  "Set the last plugin error. error-info should be {:error string :code string} or nil"
  [error-info]
  (swap! state assoc-in [:chat :last-plugin-error] error-info))

(defn clear-last-plugin-error! []
  (swap! state assoc-in [:chat :last-plugin-error] nil))
...
(defn set-hot-loaded! [plugin-id plugin-def code]
  ...
  ;; Clear any previous plugin error on successful load
  (clear-last-plugin-error!))
