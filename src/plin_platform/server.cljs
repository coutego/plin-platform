...
<body class=\"bg-gray-100\">
    ...
    <script>
        ...
        if (headConfig) {
            console.log(\"📦 Injecting head resources from manifest config...\");
            
            // C1. Inject blocking scripts
            ...
            // C4. Configure Tailwind (if tailwind object exists)
            if (headConfig.tailwind && window.tailwind) {
                console.log(\"🎨 Applying Tailwind config from manifest...\");
  ...
  (let [clean-url (first (str/split url #"\?"))]
    (let [candidates
          (cond
            ...
            ;; C. Root (Index) - User override takes priority
