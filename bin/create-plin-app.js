...
// Resolve the path to plin-platform package (works both when run from repo and from npm)
function getPlinPlatformRoot() {
  ...
  if (fs.existsSync(repoCheck)) {
    try {
      ...
      if (pkg.name === 'plin-platform') {
        return path.join(__dirname, '..');
      }
    } catch (e) {}
  }
  
  // Otherwise, we're installed as a dependency
  return path.join(__dirname, '..');
}
...
async function main() {
  try {
    ...
    const packageJson = {
      ...
      scripts: {
        start: withJsx 
          ? "npm run build:plugins && node node_modules/plin-platform/bin/generate-server-boot.js --run"
          : "node node_modules/plin-platform/bin/generate-server-boot.js --run",
...
`;
    ...
    // 6. Create JSX/TSX infrastructure if requested
    if (withJsx) {
      // esbuild config
      const esbuildConfig = `/**
...
`;
      ...
      // Copy documentation files from plin-platform
      const plinPlatformRoot = getPlinPlatformRoot();
      
      // Copy README-JS.org if it exists
      ...
      // Copy AGENTS-JS.md if it exists (for AI assistants)
...
`);
    ...
    if (withJsx) {
      ...
      console.log('  README-JS.org   - Comprehensive guide for JS/TSX plugin development');
      console.log('  AGENTS-JS.md    - Quick reference for AI coding assistants');
      console.log('');
    }
